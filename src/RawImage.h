...
RawImage<type> * newImage(const char * path){
    ...
    // Serial.println("Image File open for read.");
    int32_t size = f.size();
    // Serial.println("Image File size." + String(size));
    raw * mem = (raw *)new uint16_t[size];
    if (mem == nullptr){
    ...
    while (bytesRead < size) {
        int32_t chunkSize = min(size - bytesRead, 512); // Read in 512-byte chunks
        int32_t readResult = f.read(((uint8_t*)mem) + bytesRead, chunkSize);
        if (readResult <= 0) {
    ...
    // Serial.println("Closing Image File.");
    f.close();
    // Serial.println("Image File Closed.");
    // imgType(mem);
...
void imgType(Raw8 * var){
    Serial.print("Its Raw8");
}

void imgType(Raw16 * var){
    Serial.print("Its Raw16");
}
